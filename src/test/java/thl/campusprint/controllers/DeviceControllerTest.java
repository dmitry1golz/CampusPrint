...
/**
 * Testklasse für den DeviceController.
 * * ZWECK:
 * Dieser Test prüft die "Web-Schicht" (Controller) unserer Anwendung isoliert.
 * Das bedeutet: Wir starten NICHT den kompletten Server und wir verbinden uns NICHT
 * mit einer echten Datenbank. Stattdessen simulieren ("mocken") wir die Datenbank.
 * * TECHNOLOGIEN:
 * 1. @WebMvcTest: Startet eine abgespeckte Spring-Umgebung, die nur für Controller
 * zuständig ist. Das macht den Test extrem schnell (< 1 Sekunde).
 * * 2. MockMvc: Ein Werkzeug, das einen Webbrowser simuliert. Wir können damit
 * HTTP-Anfragen (GET, POST, DELETE) an unseren Controller senden, ohne
 * das Netzwerk wirklich zu nutzen.
 * * 3. @MockBean (DeviceRepository): Da keine echte Datenbank da ist, erstellen wir
 * eine Attrappe (Mock) des Repositories. Wir sagen dem Mock vorher genau,
 * wie er sich verhalten soll (z.B. "Wenn jemand nach ID 1 sucht, gib diesen Drucker zurück").
 * * WAS WIRD GETESTET?
 * - Mapping: Reagiert der Controller auf die richtigen URLs (/api/devices)?
 * - Datenfluss: Nimmt der Controller JSON an und wandelt es korrekt in Java-Objekte um?
 * - Antworten: Sendet der Controller das richtige JSON und den richtigen HTTP-Status (200 OK, 404 Not Found) zurück?
 * - Logik: Ruft der Controller die richtigen Methoden im Repository auf?
 */
